from sdbx import sd1_clip
from sdbx import sdxl_clip
...
class T5XXLModel(sd1_clip.SDClipModel):
    def __init__(self, device="cpu", layer="last", layer_idx=None, dtype=None):
        ...
        super().__init__(device=device, layer=layer, layer_idx=layer_idx, textmodel_json_config=textmodel_json_config, dtype=dtype, special_tokens={"end": 1, "pad": 0}, model_class=sdbx.t5.T5)
...
class SD3ClipModel(torch.nn.Module):
    def __init__(self, clip_l=True, clip_g=True, t5=True, dtype_t5=None, device="cpu", dtype=None):
        ...
        if t5:
            if dtype_t5 is None:
                dtype_t5 = dtype
            elif sdbx.model_management.dtype_size(dtype_t5) > sdbx.model_management.dtype_size(dtype):
                dtype_t5 = dtype

            if not sdbx.model_management.supports_cast(device, dtype_t5):
                dtype_t5 = dtype
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if len(token_weight_pairs_g) > 0 or len(token_weight_pairs_l) > 0:
            if self.clip_l is not None:
                ...
                l_pooled = torch.zeros((1, 768), device=sdbx.model_management.intermediate_device())
            ...
            if self.clip_g is not None:
                ...
                g_out = None
                g_pooled = torch.zeros((1, 1280), device=sdbx.model_management.intermediate_device())
        ...
        if out is None:
            out = torch.zeros((1, 77, 4096), device=sdbx.model_management.intermediate_device())

        if pooled is None:
            pooled = torch.zeros((1, 768 + 1280), device=sdbx.model_management.intermediate_device())
