...
export class ComfySplitButton {
	...
	constructor({ primary, mode, horizontal = "left", position = "relative" }, ...items) {
		...
		this.element = $el("div.sdbxui-split-button" + (mode === "hover" ? ".hover" : ""), [
			$el("div.sdbxui-split-primary", primary.element),
			$el("div.sdbxui-split-arrow", this.arrow.element),
		]);
		this.popup = new ComfyPopup({
			target: this.element,
			container: position === "relative" ? this.element : document.body,
			classList: "sdbxui-split-button-popup" + (mode === "hover" ? " hover" : ""),
