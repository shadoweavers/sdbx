...
export function createImageHost(node) {
	const el = $el("div.sdbx-img-preview");
	let currentImgs;
	let first = true;
	...
	function updateSize() {
		...
		if (currentImgs) {
			...
			if (first) {
				...
				el.style.setProperty("--sdbx-widget-min-height", elH);
			} else {
				el.style.setProperty("--sdbx-widget-min-height", null);
			}
			...
			el.style.setProperty("--sdbx-img-preview-width", w);
			el.style.setProperty("--sdbx-img-preview-height", h);
		}
	}
	return {
		el,
