...
export function toggleSwitch(name, items, { onChange } = {}) {
	...
	function updateSelected(index) {
		if (selectedIndex != null) {
			elements[selectedIndex].classList.remove("sdbx-toggle-selected");
		}
		onChange?.({ item: items[index], prev: selectedIndex == null ? undefined : items[selectedIndex] });
		selectedIndex = index;
		elements[selectedIndex].classList.add("sdbx-toggle-selected");
	}
	...
	const container = $el("div.sdbx-toggle-switch", elements);
